...
The FPKIMA has developed [draft PQC Certificate and CRL Profiles]({{site.baseurl}}/docs/fpki-x509-cert-profiles-pqc.pdf){:rel="noopener noreferrer"}{:class="usa-link"} to allow potential CITE participants the ability to develop appropriate CA and end entity certificates that leverage draft PQC cryptography.  Test certificate policy object identifiers (OIDs) are consistent with those defined in [Appendix A](#appendix-a---test-policy-object-identifiers){:class="usa-link"} below for consistency, however, other relevant certificate fields such as signature algorithm and public key have been updated to faciltiate PQC test cases.
